...
```
TODOをスキャンして表示:
taiyou__todos({ path: "./src" })

TODOを検出して自動的にIssue作成:
taiyou__todos({ path: "./src", autoCreate: true })
...
```bash
# TODOをスキャンして表示
npx taiyou todos --path ./src

# 自動的にIssue作成
npx taiyou todos --path ./src --auto-create

# 特定のパターンのみ検出
npx taiyou todos --path . --pattern "TODO|FIXME"

# ドライラン（Issue作成しない）
npx taiyou todos --path ./src --auto-create --dry-run
```
...
推奨アクション:
  1. npx taiyou todos --auto-create でIssue化
  2. 高優先度(P0/P1)から対応
```
...
🤖 Generated by Taiyou Todos Auto Mode
...
Claude: [taiyou__todos({ path: "./src" }) を実行]
...
Claude: [taiyou__todos({ path: ".", autoCreate: true }) を実行]
...
- **Label付与**: 自動付与されるLabelは `.taiyou.yml` でカスタマイズ可能
